...
import SimpsonsListItem from "./SimpsonsListItem";

class SimsonsList extends Component {
    constructor(props) {
        super(props);
    ...
    renderListItem(quote) {
        if(!this.state.filtered){
            return  <SimpsonsListItem quote={quote} />
        }

        if(quote.character.includes('Simpson')){
            return  <SimpsonsListItem quote={quote} />
        }
    }
    ...
    render() {
        return (
            <div>
                <button type="button" onClick={ this.filter }>Afficher les Simpsons</button>
...
SimsonsList.propTypes = {};

export default SimsonsList;